...
function updateManifest(){
	...
	updateManifestAppConstrains(mafApplication, manifestFile)
}

function updateManifestAppConstrains(mafApplication, manifestFile){
	var is = null;
	var os = null;
	try {
		...
		var hasEmm = hasEmmConfig(mafApplication);
		if(hasEmm){
			addEmmConfig(xmlDoc);
		}else{
			removeEmmConfig(xmlDoc);
		}
...
function hasEmmConfig(mafApplication) {
	var is = null;
	var os = null;
	try {
		...
		return xpath.evaluate("/application/emmAppConfig", xmlDoc, javax.xml.xpath.XPathConstants.NODE) !== null;
	}catch (e){
		antHelper.echo(e);
	}finally{
		if (is != null) {
...
function addEmmConfig(xmlDoc){
	var xpath = javax.xml.xpath.XPathFactory.newInstance().newXPath();
	xpath.setNamespaceContext(androidNamespace);
	parentNode = xpath.evaluate("/manifest/application", xmlDoc, javax.xml.xpath.XPathConstants.NODE);
	var nodeToAdd = xpath.evaluate("/manifest/application/meta-data[@name=\"android.content.APP_RESTRICTIONS\"]", xmlDoc, javax.xml.xpath.XPathConstants.NODE);
	if(!nodeToAdd){
		antHelper.echo("adding meta-data[@name=\"android.content.APP_RESTRICTIONS\"]");
		xmlUpdater.appendBufferAsNode(parentNode, new java.lang.String("<meta-data android:name=\"android.content.APP_RESTRICTIONS\" android:resource=\"@xml/app_restrictions\"/>"));
	}
}

function removeEmmConfig(xmlDoc){
	var xpath = javax.xml.xpath.XPathFactory.newInstance().newXPath();
	xpath.setNamespaceContext(androidNamespace);
	parentNode = xpath.evaluate("/manifest/application", xmlDoc, javax.xml.xpath.XPathConstants.NODE);
	var nodeToRemove = xpath.evaluate("/manifest/application/meta-data[@name=\"android.content.APP_RESTRICTIONS\"]", xmlDoc, javax.xml.xpath.XPathConstants.NODE);
	if(nodeToRemove){
		antHelper.echo("removing meta-data[@name=\"android.content.APP_RESTRICTIONS\"]");
